...
def inference_video(video_file, return_json=False, config_path=DEFAULT_CONFIG, model_file=DEFAULT_MODEL_FILE, params_file=DEFAULT_PARAMS_FILE):
    """完整的视频推理函数，集成了自定义预处理功能，可返回HTML或JSON"""
    # 检查视频文件
    if video_file is None:
        msg = "请上传视频文件"
        return {"error": msg, "status": "error"} if return_json else f'<div style="padding: 15px; border-radius: 10px; background-color: #fff3e0; border: 2px solid #ff9800"><h3 style="margin-top: 0; color: #e65100">{msg}</h3></div>'
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                error_msg = f"处理失败: 原始方法: {str(preprocessing_error)}; 自定义方法: {str(custom_error)}"
                return {"error": error_msg, "traceback": error_trace, "status": "error"} if return_json else f'<div style="padding: 15px; border-radius: 10px; background-color: #ffebee; border: 2px solid #ff5252"><h3 style="margin-top: 0; color: #c62828">处理失败</h3><p>原始方法: {str(preprocessing_error)}</p><p>自定义方法: {str(custom_error)}</p></div>'
        ...
        if class_match and score_match:
            ...
            if return_json:
                return {
                    "is_violent": class_id == 1,
                    "class_id": class_id,
                    "confidence": confidence,
                    "result_text": violence_text,
                    "status": "success"
                }
            else:
                # 返回结果HTML
            ...
            error_msg = f"无法解析检测结果: {output_text}"
            return {"error": error_msg, "raw_output": output_text, "status": "error"} if return_json else f'<div style="padding: 15px; border-radius: 10px; background-color: #fff3e0; border: 2px solid #ff9800"><h3 style="margin-top: 0; color: #e65100">无法解析检测结果</h3><p>原始输出: {output_text}</p></div>'
    
    except Exception as e:
        import traceback
        error_trace = traceback.format_exc()
        error_msg = f"发生错误: {str(e)}"
        return {"error": error_msg, "traceback": error_trace, "status": "error"} if return_json else f'<div style="padding: 15px; border-radius: 10px; background-color: #ffebee; border: 2px solid #ff5252"><h3 style="margin-top: 0; color: #c62828">发生错误</h3><p>{str(e)}</p><details><summary>详细信息</summary><pre>{error_trace}</pre></details></div>'
...
# 创建API端点
@app.route('/api/detect_violence', methods=['POST'])
def api_detect_violence():
    """API接口：接收视频文件并返回检测结果"""
    try:
        # 检查是否有文件上传
        if 'video' not in request.files:
            return jsonify({"error": "未找到视频文件", "status": "error"}), 400
            
        video_file = request.files['video']
        if video_file.filename == '':
            return jsonify({"error": "未选择视频文件", "status": "error"}), 400
        ...
        # 调用推理函数并返回JSON结果
        result = inference_video(video_path, return_json=True)
...
# 同时运行Gradio界面和Flask API
def run_flask():
    """在单独线程中运行Flask API服务器"""
